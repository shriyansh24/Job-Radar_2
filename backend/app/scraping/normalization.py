"""Company / title / location normalization utilities."""
...
class TitleNormalizer:
    def normalize(self, title: str | None) -> str:
        ...
        tokens = cleaned.split()
        expanded = [TITLE_ABBREVIATIONS.get(token, token) for token in tokens]
...
class LocationNormalizer:
    def normalize(self, location: str | None) -> str:
        ...
        tokens = cleaned.split()
        expanded = [LOCATION_ALIASES.get(token, token) for token in tokens]
        return " ".join(expanded)
