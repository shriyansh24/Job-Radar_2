...
class JobService:
    ...
    async def update_job(self, job_id: str, data: JobUpdate, user_id: uuid.UUID) -> Job:
        ...
        # Optimize single-record update: Use update().returning() instead of select -> setattr
        # -> commit -> refresh to minimize database roundtrips and improve throughput for updates.
        stmt = (
            update(Job)
