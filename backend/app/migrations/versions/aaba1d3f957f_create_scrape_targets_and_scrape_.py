...
def upgrade() -> None:
    ...
    op.create_index('idx_targets_active', 'scrape_targets', ['enabled', 'quarantined'], unique=False)
    op.create_index('idx_targets_ats', 'scrape_targets', ['ats_vendor'], unique=False)
    op.create_index('idx_targets_schedule', 'scrape_targets', ['priority_class', 'next_scheduled_at'], unique=False, postgresql_where=sa.text('enabled = TRUE AND quarantined = FALSE'))
...
def downgrade() -> None:
    ...
    op.drop_index('idx_targets_schedule', table_name='scrape_targets', postgresql_where=sa.text('enabled = TRUE AND quarantined = FALSE'))
