"""Tests for the ATS Resume Validator (B4)."""
...
class TestATSValidator:
    ...
    def test_missing_email_flagged(self):
        ...
        email_check = next(c for c in result.checks if c.field == "contact_email")
    ...
    def test_missing_phone_flagged(self):
        ...
        validator = ATSValidator()
        result = validator.validate(ir)
        phone_check = next(c for c in result.checks if c.field == "contact_phone")
    ...
    def test_no_work_section_flagged(self):
        ...
        validator = ATSValidator()
        result = validator.validate(ir)
        work_check = next(c for c in result.checks if c.field == "has_work")
    ...
    def test_no_skills_flagged(self):
        ...
        validator = ATSValidator()
        result = validator.validate(ir)
        skills_check = next(c for c in result.checks if c.field == "has_skills")
    ...
    def test_no_summary_flagged(self):
        ...
        validator = ATSValidator()
        result = validator.validate(ir)
        summary_check = next(c for c in result.checks if c.field == "has_summary")
    ...
    def test_bullet_quality_with_good_bullets(self):
        ...
        validator = ATSValidator()
        result = validator.validate(ir)
        quality_check = next(c for c in result.checks if c.field == "bullet_quality")
    ...
    def test_text_length_check(self):
        validator = ATSValidator()
        result = validator.validate(_make_full_ir())
        length_check = next(c for c in result.checks if c.field == "text_length")
    ...
    def test_standard_headers_all_present(self):
        validator = ATSValidator()
        result = validator.validate(_make_full_ir())
        headers_check = next(c for c in result.checks if c.field == "standard_headers")
