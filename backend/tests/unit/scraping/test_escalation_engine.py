from app.scraping.execution.escalation_engine import EscalationReason, should_escalate


def test_403_triggers_escalation():
    assert should_escalate(status_code=403, jobs_found=0, html_length=0)
    assert (
        should_escalate(status_code=403, jobs_found=0, html_length=0).reason
        == EscalationReason.HTTP_FORBIDDEN
    )
...
def test_cloudflare_challenge_detected():
    result = should_escalate(
        status_code=403, jobs_found=0, html_length=2000, html_snippet="Checking your browser"
    )
    assert result
...
def test_timeout_triggers_escalation():
    result = should_escalate(status_code=None, jobs_found=0, html_length=0, timed_out=True)
    assert result
