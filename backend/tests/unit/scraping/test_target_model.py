...
def test_scrape_target_has_required_columns():
    ...
    required = {
        "id", "user_id", "url", "company_name", "ats_vendor",
...
def test_scrape_target_has_additional_columns():
    ...
    columns = {c.name for c in ScrapeTarget.__table__.columns}
    additional = {
        "company_domain", "source_kind", "quarantine_reason",
        "last_success_at", "last_failure_at", "last_success_tier",
        "last_http_status", "etag", "last_modified",
        "failure_count", "industry", "created_at", "updated_at",
    }
