...
- Scheduler isolation is now queue-backed through ARQ, and queue telemetry now includes depth, oldest-job age, alert state, truthful `retry_exhausted` final-failure logs, request/job correlation on queue-triggered operator paths, Redis-backed telemetry history, and alert-transition events.
...
- Auth lifecycle logging now carries request correlation and normalized reason codes through the main app log stream, and the repo now supports a dedicated Redis-backed auth audit stream with Admin-visible recent event history when configured.
